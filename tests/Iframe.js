...
step("Navigate to Iframe page", async function () {
    try {
        await highlight($(`//*[@id='menuform:j_idt37']`));
        await click($(`//*[@id='menuform:j_idt37']`));
        await click($(`//*[@id='menuform:m_frame']`));
    } catch (error) {
        console.error(error);
    }
 })
 ...
 step("click on Iframe", async function () {
    try {
        await highlight($(`//*[@id='Click']`));
        await click($(`//*[@id='Click']`));
    } catch (error) {
        console.error(error);
    }
 })
...
step("click on nested IFrame", async function () {
    try {
       // await switchTo({"id":"frame2"})
        await highlight($(`//iframe[@id='frame2']`));
        await click($(`//iframe[@id='frame2']`), within("Click Me"));
    } catch (error) {
        console.error(error);
    }
 })
