...
step("Navigate to checkBox page", async function () {
    try {
        await highlight($(`//*[@id='menuform:j_idt38']`));
        await click($(`//*[@id='menuform:j_idt38']`));
        await click($(`//*[@id='menuform:m_checkbox']`));
    } catch (error) {
        console.error(error);
    }
 })

 step("click on Checkbox", async function () {
    try {
        await highlight($(`//*[@id='j_idt87:j_idt89']`));
        await checkBox({"id":"j_idt87:j_idt89_input"}).check();
